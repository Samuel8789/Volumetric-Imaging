...
[px py]=meshgrid(50:-50:-50, 50:-50:-50);
xyp=[py(:) px(:)];
theta=-0/180*pi;
...
% automatic acquision, start NI instrument and actuate SLM
    ...
    waitfor(msgbox(['Please set up the Microscope to acquire ' num2str(size(xyzp,1)) ' images, and connect D0 port in NI box to Microscope input trigger. Start Microscope Acquisition. Press OK when ready.']));
    for idx=1:size(xyzp,1)
        handles.acqImageID=idx;
        f_SLMActivation_Calibration( handles.SLM_handles, handles.xyzp(idx,:), weight, handles.objectiveNA, handles );
    ...
    handles.acqImageID=0;
    weight=1;
    f_SLMActivation_Calibration( handles.SLM_handles, [handles.SLMPreset handles.currentFocus], weight, handles.objectiveNA, handles );     
else
% manual acquision
    handles.acqImageID=1;
    set(handles.AcqImageID_text, 'String', [num2str(handles.acqImageID) ' of ' num2str(handles.imageNum)]);
    f_SLMActivation_Calibration( handles.SLM_handles, handles.xyzp(handles.acqImageID,:), weight, handles.objectiveNA, handles );     
end
guidata(hObject, handles);
...
% calculate the transformation matrix
 ...
 xy(:,1)=xy(:,1);   % note: x, y is swapped
 xy(:,2)=xy(:,2);   % note: x, y is swapped
 
 % Matrix transformation, from xyp to xy
 xyp=handles.xyzp(:,1:2);
 handles.tLaserSLM=cp2tform(xyp, xy, 'affine');
...
calImageSize=handles.imageSize;

save([path file],'tLaserSLM','focusPlane','shiftedImageTargetList','refImageTargetList','calImageSize');
...
if handles.autoZoom==1
    zoneX=handles.imageSize(1)-handles.imageSize(1)/handles.gridSize*(mod(handles.calImageID-1,handles.gridSize)+1)+handles.imageSize(1)/handles.gridSize/2;
    zoneY=handles.imageSize(1)/handles.gridSize*(floor((handles.calImageID-1)/handles.gridSize)+1)-handles.imageSize(1)/handles.gridSize/2;    
    axes(handles.ShiftedImage_axe);
    axis([zoneX-handles.imageSize(1)/handles.gridSize zoneX+handles.imageSize(1)/handles.gridSize zoneY-handles.imageSize(1)/handles.gridSize zoneY+handles.imageSize(1)/handles.gridSize]);
else
    axes(handles.ShiftedImage_axe);
...
interX=handles.imageSize(1)/handles.gridSize;
interY=handles.imageSize(1)/handles.gridSize;
zoneX=handles.imageSize(1)-handles.imageSize(1)/handles.gridSize*(mod(handles.calImageID-1,handles.gridSize)+1)+handles.imageSize(1)/handles.gridSize/2;
zoneY=handles.imageSize(1)/handles.gridSize*(floor((handles.calImageID-1)/handles.gridSize)+1)-handles.imageSize(1)/handles.gridSize/2;    
